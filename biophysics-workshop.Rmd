...
* Different directions of vibration (called 'polarization') can be filtered out using a polarizing filter. A polarizing filter is like a fence, and the light will get blocked if it isn't lined up in the same direction as the fence slats. 
...
**Ask students**: get them to theorize about why zebras have stripes, and why there are so many different kinds of trees. There are no right answers at this point.
...
* Coloured tokens in two colours for each pod (these could be anything - chits, balls, paper pieces) - about 10 of each per group. We used coloured coins in green and purple.
...
* Draw histrogram axes on the board. Ask each group to report the number of times that each colour won, and add the totals to the histogram as you go. **Ask students** what they notice about the result. Is it equal? Why or why not?
